...
#    DivNum_x_type1, DivNum_x_type2 = 100, 20
    ...
    DivNum_r = 200
    DivNum_z = 200
    DivNum_x_type1, DivNum_x_type2 = 200, 40
    ...
    def func_rho(tier_i, y, r, u, theta):
        ...
        if not y or not r or not u:
            ...
            epnt_part2 = integrate.trapezoid(integd_x(L_x), L_x)
    ...
    def func_K_mod(tier_i, y, r, u, theta, z):
        ...
        return np.exp( -mb * integrate.trapezoid(integd_x_cv(L_x_cv_truncated), L_x_cv_truncated) )
    ...
    def func_A(tier_i, r):
        ...
        return np.exp( -2*np.pi*lam2p * integrate.trapezoid(integd_z_cv(L_z_cv_truncated, r), L_z_cv_truncated) )
    ...
    def func_J(tier_i, y, r, u, theta):
        ...
        return func_A(tier_i, r) * np.exp( -2*np.pi*lam2p * integrate.trapezoid(integd_z_cv(L_z_cv, r), L_z_cv) )
    ...
    def summand_DR_term1(t1):
            ...
            def integd_y(y):
                def integd_r_cv(L_r_cv, y):
                    def integd_r(r, y):
                        ...
                        return r*np.exp(-lam1*np.pi*r**2)*integrate.trapezoid(integd_theta(L_theta, r, y), L_theta)
                    L_r = np.tan(np.pi/2*L_r_cv)
                    return np.array([ np.pi/2*(1 + r**2)*integd_r(r, y) for r in L_r ])
                L_r_cv = np.linspace(10**(-8), 1, DivNum_r+1)
                return integrate.trapezoid(integd_r_cv(L_r_cv, y), L_r_cv)/(1 + y)
        ...
        return 2*lam1*integrate.trapezoid(integd_y_cv(L_y_cv), L_y_cv)
            ...
            def integd_y(y):
                def integd_r_cv(L_r_cv, y):
                    def integd_r(r, y):
                        ...
                        L_theta = np.linspace(0, np.pi, DivNum_theta+1)
                        return np.exp(-lam1*np.pi*P_bar(2, 1)**2*r**2)*func_B(r)*integrate.trapezoid(integd_theta(L_theta, r, y), L_theta)
                    L_r = np.tan(L_r_cv)
                ...
                return integrate.trapezoid(integd_r_cv(L_r_cv_truncated, y), L_r_cv_truncated)/(1 + y)
            L_y = np.exp(np.sinh(L_y_cv)*np.pi/2)
            return np.array([ (np.cosh(y_cv)*np.pi/2)*y*integd_y(y) for y_cv, y in zip(L_y_cv, L_y) ])
        y_cv_min = -3.0; y_cv_max = 3.0
        L_y_cv = np.linspace(y_cv_min, y_cv_max, DivNum_y+1)
        return mb/np.pi*integrate.trapezoid(integd_y_cv(L_y_cv), L_y_cv)
